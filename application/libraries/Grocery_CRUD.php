	...
	protected function db_insert($state_info)
	{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert === null)
			{
				...
				{
					$post_data['table_name'] = $this->basic_model->get_basic_table();
			...
			foreach($this->where as $where)
                ...
                if(!empty($this->where_in))
                        foreach($this->where_in as $where_in)
                                $this->basic_model->where($where_in[0].' in ('.$where_in[1].')'); //ajout adam
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected $edit_custom_redirection              = null;
	protected $add_custom_redirection               = null;
	protected $delete_custom_redirection               = null;

	public function edit_custom_redirection($url)
	{
		$this->edit_custom_redirection = $url;
	}

	public function add_custom_redirection($url)
	{
		$this->add_custom_redirection = $url;
	}
	
	public function delete_custom_redirection($url)
	{
		$this->delete_custom_redirection = $url;
	}
	...
	protected function showEditForm($state_info)
	{
		...
		$data->unique_hash			= $this->get_method_hash();
                $data->cle_eleve = $state_info->primary_key;
	...
	{
		@ob_end_clean();
		if (isset($this->delete_custom_redirection)){
			echo 
				"<script>
					document.location.replace('" . $this->delete_custom_redirection . "');
				</script>";
		}
		echo
			"<script>
	...
	protected function insert_layout($insert_result = false)
	{
		...
		{
			$success_message = '<div class="col-xs-12"><center><i class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></i></center></div>';
			if (isset($this->add_custom_redirection)){
				$success_message .= 
					"<script>
						document.location.replace('" . $this->add_custom_redirection[0] . $this->getStateInfo()->unwrapped_data[$this->add_custom_redirection[1]] . $this->add_custom_redirection[2] . "');
					</script>";
			} else {
				$success_message .= 
					"<script>
			...
			else if (isset($this->add_custom_redirection))
			{
				$success_message .= " <a href='". $this->add_custom_redirection[0] . $this->getStateInfo()->unwrapped_data[$this->add_custom_redirection[1]] . $this->add_custom_redirection[2] . "'>Retourner au dossier</a>";
			}
	...
	protected function update_layout($update_result = false, $state_info = null)
	{
		...
		{
			$success_message = '<div class="col-xs-12"><center><i class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></i></center></div>';
			if (isset($this->edit_custom_redirection)){
				$success_message .=
					"<script>
						document.location.replace('" . $this->edit_custom_redirection . "');
					</script>";
			} else {
				$success_message .= 
					"<script>
			...
			else if (isset($this->edit_custom_redirection))
			{
				$success_message .= " <a href='".$this->edit_custom_redirection."'>Retourner au dossier</a>";
			}
			$success_message .= '</p>';
			*/
			echo json_encode(array(
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='form-control datepicker' />";
		return $input;
	}
	...
	protected function get_read_input_fields($field_values = null)
	{
		...
		foreach ($read_fields as $field) {
			if (!empty($this->change_field_type)
					...
					&& $this->change_field_type[$field->field_name]->type == '') {
				break;
			} else {
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $where 				= array();
        protected $where_in = array();//ajout adam
	...
	protected $default_config_path		= 'assets/plugins/grocery_crud/config';
	protected $default_assets_path		= 'assets/plugins/grocery_crud';
	...
	public function or_where($key, $value = NULL, $escape = TRUE)
	{
        ...
        public function where_in($key, $value = NULL, $escape = TRUE)//ajout adam
        {
                $this->where_in[] = array($key,$value,$escape);
                return $this;
        }
	...
	protected function _initialize_variables()
	{
		...
		if ($this->character_limiter === 0 || $this->character_limiter === '0') {
			...
			$this->character_limiter = 50; //is better to have the number 30 rather than the 0 value
		}
