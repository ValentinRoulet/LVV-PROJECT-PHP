...
function updateAdditionalAuthorList()
{
    ...
    for( var i = 0 ; i < g_additionalAuthorList.length ; ++i )
    {
        ...
        btn.onclick = ( function(entry) { removeAdditionalAuthor( this.name );  } );
...
function updateInformantTable()
{
    ...
    for( var i = 0 ; i < g_informants.length; ++i )
    {
        ...
        btn.setAttribute("value","X");
        btn.setAttribute("name" , "" + i );
        btn.onclick = ( function(entry) { console.log("erase element "+ (this.name) ) ; removeInformant( this.name );  } );
        c4.appendChild( btn );
    }
}
...
function updateEventCodeTable()
{
    ...
    for( var i = 0 ; i < g_eventCodes.length ; ++i )
    {
        ...
        btn.setAttribute("value","X");
        btn.setAttribute("name" , "" + i );
        btn.onclick = ( function(entry) { console.log("erase element "+ (this.name) ) ; removeEventCode( this.name );  } );
        c4.appendChild( btn );
    }
}
...
function updateDocumentsTable()
{
    ...
    for( var i = 0 ; i < g_documents.length ; ++i )
    {
        ...
        btn.setAttribute("value", "X");
        btn.setAttribute("name", "" + i);
        btn.onclick = (function (entry) { console.log("Erase document element " + (this.name)); removeDocument(this.name); });
...
function listPcscSmartcardReaders()
{
    ...
    var tseCredentials = document.getElementById(  "TseCredentials" );        if( tseCredentials )        {            if( tseCredentials.value.length > 0 )            {                if( optionalParameters == undefined )                {                    optionalParameters = {};                }                                optionalParameters.s_tseCredentials = tseCredentials.value;            }        }      
...
function getServiceVersion() 
{
    ...
    dmpConnectInstance.sendHttpCommand( command , "getServiceVersion" , function (e)
    {
        if( e.error )
        {
            appendLog( "/getServiceVersion failed: ", e );
        }
        else
        {
            appendLog( "/getServiceVersion succeeded: ", e );
        }
    } );    
}
...
{
    optionalParameters = {};
    optionalParameters.i_getDcparamInformations = 1;
...
function hl_getInteropCodes()
{
    var params =
    {
       ...
       s_codeIds: [
           ...
           "physicianSpecialities",     // Specialities for 'Médecin' profession (code: 10).
    ...
    dmpConnectInstance.hl_getInteropCodes(params, function(a) {
        if (a.error) {
            appendLog("-- hl_getInteropCodes failed: ", a);
        } else {
            appendLog("-- hl_getInteropCodes succeeded.", a );
        }
    });
}

function hl_getUpdatedTypeCode()
{
    dmpConnectInstance.hl_getUpdatedTypeCode( "10213-7", function(a) {
        if( a.error ) {
            appendLog( "-- hl_getUpdatedTypeCode failed: ", a );
        } else {
            appendLog( "-- hl_getUpdatedTypeCode succeeded: ", a );
        }
    });
}


function hl_convertLunarDateToStandardDate() 
{
    var inDate = getFormTextFieldValue(document.getElementById("inputDate"));
    dmpConnectInstance.hl_convertLunarDateToStandardDate(inDate, function (a) {
        if (a.error) {
            appendLog("-- hl_convertLunarDateToStandardDate failed: ", a);
        }
        else {
            appendLog("-- hl_convertLunarDateToStandardDate succeeded.", a);
            setTextValue("outputDate", a.s_date);
        }
    }
    );
}

function hl_computeBirthDayCentury() 
{
    var inDate          = getFormTextFieldValue(document.getElementById("inputBirthday"));
    var qualitySelector = document.getElementById("patientQualitySelector");
    var quality         = qualitySelector.value;

    dmpConnectInstance.hl_computeBirthDayCentury(inDate, quality, function (a) {
        if (a.error) {
            appendLog("-- hl_computeBirthDayCentury failed: ", a);
        }
        else {
            appendLog("-- hl_computeBirthDayCentury succeeded.", a);
            setTextValue("birthdayResult", a.i_century);
        }
    });
}

function hl_regularizeBirthDateIfNeeded() 
{
    var inDate          = getFormTextFieldValue(document.getElementById("inputBirthday"));
    var qualitySelector = document.getElementById("patientQualitySelector");
    var quality         = qualitySelector.value;

    dmpConnectInstance.hl_regularizeBirthDateIfNeeded(inDate, quality, function (a) {
        if (a.error) {
            appendLog("-- hl_regularizeBirthDateIfNeeded failed: ", a);
        }
        else {
            appendLog("-- hl_regularizeBirthDateIfNeeded succeeded.", a);
            setTextValue("birthdayResult", a.s_date);
        }
    });
}

var g_remoteControlMonitoringWS;

function hl_startRemoteControl()
{
    dmpConnectInstance.hl_startRemoteControl( null, function (e)
    {
        if( e.error )
        {
            appendLog( "-- hl_startRemoteControl failed: ", e );
        }
        else 
        {
            appendLog( "-- hl_startRemoteControl succeeded. ", e );
        }
    });
}

function hl_stopRemoteControl()
{
    dmpConnectInstance.hl_stopRemoteControl( null, function (e) 
    {
        if( e.error )
        {
            appendLog( "-- hl_stopRemoteControl failed: ", e );
        }
        else 
        {
            appendLog( "-- hl_stopRemoteControl succeeded. ", e );
        }
    });
}

function hl_remoteControlStatus() 
{
    dmpConnectInstance.hl_remoteControlStatus( null , function (e) 
    {
        if( e.error )
        {
            appendLog( "-- hl_remoteControlStatus failed: ", e );
        }
        else 
        {
            appendLog( "-- hl_remoteControlStatus succeeded. ", e );
        }
    });
}

function hl_startRemoteControlInMonitoring()
{
    g_remoteControlMonitoringWS = dmpConnectInstance.hl_startRemoteControlInMonitoring( function( receivedObject, responseCallback )
    {
        if( receivedObject.error )
        {
            appendLog( "-- hl_startRemoteControlInMonitoring failed: ", receivedObject );
        }
        else 
        {
            appendLog( "-- New Remote IN command: ", printObject( receivedObject ) + "\n" );
...
function hl_sendRemoteInOkAnswer( )
{
    var requestId = getTextValue( "requestId" );
    ...
    dmpConnectInstance.hl_sendRemoteInAnswer( answer , function (e) 
    {
        if (e.error) 
        {
            appendLog("-- hl_sendRemoteInAnswer failed: ", e);
        }
        else 
        {
            appendLog("-- hl_sendRemoteInAnswer succeeded: ", e);
        }
    });
}


function hl_stopRemoteControlInMonitoring()
{
    g_remoteControlMonitoringWS.close();
}

function hl_exportDocumentToRemoteOut() 
{
    dmpConnectInstance.hl_exportDocumentToRemoteOut(getDocumentHandle(), function (e) 
    {
        if (e.error) 
        {
            appendLog("-- hl_exportDocumentToRemoteOut failed.", e);
        }
        else
        {
            appendLog("-- hl_exportDocumentToRemoteOut succeeded.", e);
        }
    });
}

function hl_exportDocumentToRemoteOutByUniqueId()
{
    var uniqueId = getDocumentUniqueId();
    var uuid     = getDocumentUuid();
    var ins      = getSelectedIns();

    dmpConnectInstance.hl_exportDocumentToRemoteOutByUniqueId( ins, uniqueId, uuid , function(e) 
    {
        if( e.error )
        {
            appendLog( "-- hl_exportDocumentToRemoteOutByUniqueId failed.", e );
        }
        else 
        {
            appendLog( "-- hl_exportDocumentToRemoteOutByUniqueId succeeded.", e );
        }
    });
}

function hl_notifyPsConnectedToRemoteOut()
{
    dmpConnectInstance.hl_notifyPsConnectedToRemoteOut( function (e) 
    {
        if( e.error )
        {
            appendLog("-- hl_notifyPsConnectedToRemoteOut failed.", e );
        }
        else 
        {
            appendLog("-- hl_notifyPsConnectedToRemoteOut succeeded.", e );
        }
    });
}
...
{
    dmpConnectInstance.getLogLevels(undefined, function (a) {
        if (a.error) {
            appendLog("-- getLogLevel failed: ", a);
        }
        else 
        {
            appendLog("-- getLogLevel succeeded.", a);
        
            var optionDmpcLogLevel   = document.getElementById( "DmpcLogLevel" );
            var optionDmpcJsLogLevel = document.getElementById( "DmpcJsLogLevel" );

            optionDmpcJsLogLevel.value = a.i_dmpConnectJsLogLevel ;
            optionDmpcLogLevel.value = a.i_dmpConnectLogLevel ;
        }
    });
}
...
function setLogLevels()
{
    ...
    dmpConnectInstance.setLogLevels( undefined, parseInt(optionDmpcLogLevel.value), parseInt(optionDmpcJsLogLevel.value), function (a) {
        if (a.error) {
            appendLog("-- setLogLevels failed: ", a);
        }
        else {
            appendLog("-- setLogLevels succeeded.", a);
        }
    });
}
